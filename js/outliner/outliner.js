...
import { PrimitiveElement } from './primitives.js'
...
export const Outliner = {
	...
	// ============================================
	// Primitives - General 3D Shapes Support
	// ============================================
	primitives: {
		createSphere(options = {}) {
			let sphere = new PrimitiveElement({
				shape: 'sphere',
		...
		createCylinder(options = {}) {
			let cylinder = new PrimitiveElement({
				shape: 'cylinder',
		...
		createTorus(options = {}) {
			let torus = new PrimitiveElement({
				shape: 'torus',
				radius: options.radius || 8,
				segments: options.segments || 32,
				rings: options.rings || 32,
		...
		createPlane(options = {}) {
			let plane = new PrimitiveElement({
				shape: 'plane',
				radius: options.radius || 8,
		...
		function iterate(array, save_array, addGroup) {
			for (let item of array) {
				if (typeof item === 'string') {
					...
					// Primitive support
					if (item && item.type === 'primitive') {
						obj = new PrimitiveElement(item, item.uuid);
